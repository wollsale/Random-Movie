...
function fetchLatestMovieId() {
    ...
    return fetch(url).then((response)=>response.json()).then((data)=>{
        ...
        console.log("Erreur lors de la requ\xeate \xe0 l'API :", error);
        return null;
    });
}
function fetchRandomMovie() {
    return fetchLatestMovieId().then((latestMovieId)=>{
        if (latestMovieId) {
...
function isValidMovie(movie) {
    return movie.title && movie.release_date && movie.vote_average && movie.poster_path && movie.title !== "undefined" && movie.release_date !== "undefined" && movie.vote_average !== "undefined" && movie.poster_path !== "undefined";
}
...
function displayMovieInfo(movie) {
    ...
    } else fetchRandomMovie().then((movie)=>{
        const movieObject = createMovieObject(movie);
        displayMovieInfo(movieObject);
    }).catch((error)=>{
        console.log("Erreur lors de la r\xe9cup\xe9ration du film al\xe9atoire :", error);
    });
}
// TRIGGER
// if (fresh = false) {
